...
namespace stateless_guess_game_tests.GuessGameTests.Commands
{
    public class HelpCommandShould
    {
        ...
        public void WhisperTheGameIsTakingGuesses_WhenTakingGuesses_GivenUserIsBroadcaster()
        {
            ...
            chatserviceMock.Verify(service => service.WhisperMessage(
                ...
                It.Is<string>(m => m == "The time-guessing game is currently taking guesses.  Guess a time with !guess 1:23 OR close the guesses with !guess close")), Times.Once);
        }
        ...
        [Fact]
        public void DoNothing_WhenClosed_GivenUserIsBroadcaster()
        {
            var sut = new GuessGame(GuessGameState.GuessesClosed);
            Mock<IChatService> chatserviceMock = new Mock<IChatService>();
            chatserviceMock.Setup(x => x.BroadcastMessageOnChannel(It.IsAny<string>()));
            ...
            chatserviceMock.Verify(service => service.WhisperMessage(It.IsAny<string>(), It.IsAny<string>()), Times.Never);
        }
    }
}
