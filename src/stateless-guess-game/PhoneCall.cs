...
namespace stateless_guess_game
{
    ...
    namespace TelephoneCallExample
    {
        public class PhoneCall
        {
            ...
            public PhoneCall(string caller, ITestOutputHelper output)
            {
                _caller = caller;
                _output = output;
            ...
            {
                _output.WriteLine("Volume set to " + volume + "!");
            }

            void OnUnmute()
            {
                _output.WriteLine("Microphone unmuted!");
            }

            void OnMute()
            {
                _output.WriteLine("Microphone muted!");
            }

            void OnDialed(string callee)
            {
                _callee = callee;
                _output.WriteLine("[Phone Call] placed for : [{0}]", _callee);
            }

            void StartCallTimer()
            {
                _output.WriteLine("[Timer:] Call started at {0}", DateTime.Now);
            }

            void StopCallTimer()
            {
                _output.WriteLine("[Timer:] Call ended at {0}", DateTime.Now);
            }
            ...
            public void Print()
            {
                _output.WriteLine("[{1}] placed call and [Status: {0}]", _machine.State, _caller);
            }
